...
bar_release:
    ...
    cmp rax, rbx
    jl bar_release_if_num_curried_lt_2
    mov rbx, [rbp-16] ; load env_end pointer for release
...
bar_deepcopy:
    ...
    mov rbx, 2 ; operand literal
    cmp rax, rbx
    jl bar_deepcopy_if_num_curried_lt_2
    mov rbx, [rbp-16] ; load env_end pointer for copy
    mov rcx, [rbx-16] ; load field pointer
    test rcx, rcx ; skip null field
...
bar_deepcopy_copy_field_done_1:
    ...
    mov rcx, [rbx-8] ; load field pointer
    test rcx, rcx ; skip null field
    je bar_deepcopy_copy_field_null_2
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [rbx-8], rax ; store duplicated pointer
    jmp bar_deepcopy_copy_field_done_3
bar_deepcopy_copy_field_null_2:
    xor rax, rax ; null copy result
bar_deepcopy_copy_field_done_3:
...
bar_deepcopy_if_num_curried_lt_2:
    ...
    mov rcx, [rbx-16] ; load field pointer
    test rcx, rcx ; skip null field
    je bar_deepcopy_copy_field_null_4
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [rbx-16], rax ; store duplicated pointer
    jmp bar_deepcopy_copy_field_done_5
bar_deepcopy_copy_field_null_4:
    xor rax, rax ; null copy result
bar_deepcopy_copy_field_done_5:
...
_start:
    ...
    mov rbp, rsp ; establish new frame base
    sub rsp, 64 ; reserve stack space for locals
    lea rax, [rel msg] ; point to string literal
    ...
    mov rax, exit_unwrapper ; load unwrapper entry point
    mov qword [rdx+0], rax ; store unwrapper entry in metadata
    mov rax, exit_release ; load release helper entry point
    mov qword [rdx+8], rax ; store release pointer in metadata
    mov rax, exit_deepcopy ; load deep copy helper entry point
