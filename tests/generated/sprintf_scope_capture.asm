...
foo:
    ...
    mov rax, _foo_3_closure_entry ; load wrapper entry point
...
global _foo__foo_3__foo__foo_3_2_1
_foo__foo_3__foo__foo_3_2_1:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
...
_foo__foo_3__foo__foo_3_2_1_closure_entry:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish wrapper frame
    sub rsp, 16 ; reserve space for env metadata scratch
    mov [rbp-8], rdi ; stash env_end pointer for release
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp _foo__foo_3__foo__foo_3_2_1 ; jump into actual function
global _foo__foo_3_2
_foo__foo_3_2:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    mov qword [rdx+16], 0 ; pointer count metadata
    mov rax, _foo__foo_3__foo__foo_3_2_1_closure_entry ; load wrapper entry point
    sub rsp, 24 ; allocate temporary stack for closure state
    mov [rsp], rax ; save closure code pointer temporarily
    mov [rsp+8], rdx ; save closure env_end pointer temporarily
    mov rax, [rsp] ; restore closure code pointer
...
_foo__foo_3_2_write_strlen_loop_0:
    mov dl, byte [r8+rcx] ; load current character
    cmp dl, 0 ; stop at terminator
    je _foo__foo_3_2_write_strlen_done_0
    inc rcx ; advance char counter
    jmp _foo__foo_3_2_write_strlen_loop_0
_foo__foo_3_2_write_strlen_done_0:
...
_foo__foo_3_2_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp _foo__foo_3_2 ; jump into actual function
global _foo_3
_foo_3:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    mov qword [rdx+8], 32 ; heap size metadata
    mov qword [rdx+16], 0 ; pointer count metadata
    mov rax, _foo__foo_3_2_closure_entry ; load wrapper entry point
    sub rsp, 24 ; allocate temporary stack for closure state
    mov [rsp], rax ; save closure code pointer temporarily
    mov [rsp+8], rdx ; save closure env_end pointer temporarily
    mov rax, [rsp] ; restore closure code pointer
    ...
    xor r9, r9 ; offset = 0
    syscall ; allocate env pages
    mov r12, rax ; keep sprintf buffer pointer
    ...
    call sprintf
    add rsp, 8
    mov r10, r13 ; copy env_end pointer
    sub r10, 8 ; slot for formatted result
...
_foo_3_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp _foo_3 ; jump into actual function
global _start
_start:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
