...
identity:
    ...
    mov rax, _4_identity_release ; load release helper entry point
    mov qword [rdx+8], rax ; store release pointer in metadata
    mov rax, _4_identity_deepcopy ; load deep copy helper entry point
...
identity_release:
    ...
    cmp rax, rbx
    jl identity_release_if_num_curried_lt_1
    ...
    jmp identity_release_done
identity_release_if_num_curried_lt_1:
identity_release_done:
    leave
    ret
...
identity_deepcopy:
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jl identity_deepcopy_if_num_curried_lt_1
    mov rbx, [rbp-16] ; load env_end pointer for copy
    mov rcx, [rbx-8] ; load field pointer
    test rcx, rcx ; skip null field
    je identity_deepcopy_copy_field_null_0
    ...
    jmp identity_deepcopy_copy_field_done_1
identity_deepcopy_copy_field_null_0:
    xor rax, rax ; null copy result
identity_deepcopy_copy_field_done_1:
    mov [rbp-48], rax ; save evaluated scalar in frame
    jmp identity_deepcopy_done
identity_deepcopy_if_num_curried_lt_1:
identity_deepcopy_done:
    leave
    ret
...
hello:
    ...
    mov qword [rdx+0], rax ; store unwrapper entry in metadata
    mov rax, _9_hello_release ; load release helper entry point
    mov qword [rdx+8], rax ; store release pointer in metadata
    mov rax, _9_hello_deepcopy ; load deep copy helper entry point
    mov qword [rdx+16], rax ; store deep copy pointer in metadata
    xor rax, rax ; zero num_curried metadata
    mov qword [rdx+40], rax ; store num_curried
    mov [rbp-32], rdx ; update closure env_end pointer
