...
_32_iterate:
    ...
    leave ; unwind before jumping
    jmp rax ; tail call into closure
global _32_iterate_unwrapper
_32_iterate_unwrapper:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 48 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    leave ; unwind before named jump
    jmp _32_iterate
global _32_iterate_deep_release
_32_iterate_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 48 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 4 ; operand literal
    cmp rax, rbx
    jg _32_iterate_release_skip_0
    mov rax, [r12-40] ; load _32_iterate_release_field_0 env field
    mov [rbp-24], rax ; store value
    mov rdi, [rbp-24] ; load operand
    call release_heap_ptr ; release heap pointer
_32_iterate_release_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 3 ; operand literal
    cmp rax, rbx
    jg _32_iterate_release_skip_1
    mov rax, [r12-32] ; load _32_iterate_release_field_1 env field
    mov [rbp-32], rax ; store value
    mov rdi, [rbp-32] ; load operand
    call release_heap_ptr ; release heap pointer
_32_iterate_release_skip_1:
    mov rax, [rbp-16] ; load operand
    mov rbx, 2 ; operand literal
    cmp rax, rbx
    jg _32_iterate_release_skip_2
    mov rax, [r12-24] ; load _32_iterate_release_field_2 env field
    mov [rbp-40], rax ; store value
    mov rdi, [rbp-40] ; load operand
    call release_heap_ptr ; release heap pointer
_32_iterate_release_skip_2:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _32_iterate_release_skip_4
    mov rax, [r12-8] ; load _32_iterate_release_field_4 env field
    mov [rbp-48], rax ; store value
    mov rdi, [rbp-48] ; load operand
    call release_heap_ptr ; release heap pointer
_32_iterate_release_skip_4:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret

global _32_iterate_deepcopy
_32_iterate_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 48 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
    ...
    mov rbx, 4 ; operand literal
    cmp rax, rbx
    jg _32_iterate_deepcopy_skip_0
    mov rcx, [r12-40] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-40], rax ; store duplicated pointer
    mov [rbp-24], rax ; store value
_32_iterate_deepcopy_skip_0:
    mov rax, [rbp-16] ; load operand
    mov rbx, 3 ; operand literal
    cmp rax, rbx
    jg _32_iterate_deepcopy_skip_1
    mov rcx, [r12-32] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-32], rax ; store duplicated pointer
    mov [rbp-32], rax ; store value
_32_iterate_deepcopy_skip_1:
    mov rax, [rbp-16] ; load operand
    mov rbx, 2 ; operand literal
    cmp rax, rbx
    jg _32_iterate_deepcopy_skip_2
    mov rcx, [r12-24] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-24], rax ; store duplicated pointer
    mov [rbp-40], rax ; store value
_32_iterate_deepcopy_skip_2:
    mov rax, [rbp-16] ; load operand
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _32_iterate_deepcopy_skip_4
    mov rcx, [r12-8] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-8], rax ; store duplicated pointer
    mov [rbp-48], rax ; store value
_32_iterate_deepcopy_skip_4:
    leave
    ret
...
iterate:
    ...
    mov qword [r12+32], rax ; heap size metadata
    lea rax, [_32_iterate_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_32_iterate_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_32_iterate_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 2 ; store num_remaining
    mov rax, r12 ; copy _33_iterate closure env_end to rax
    mov [rbp-40], rax ; store value
...
handler:
    ...
    mov rax, [rbp-8] ; load operand
    push rax ; stack arg
    lea rax, [rel _41] ; point to string literal
    push rax ; stack arg
    pop rdi ; restore arg into register
    pop rsi ; restore arg into register
