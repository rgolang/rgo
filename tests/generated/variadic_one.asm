...
__lambda_1_write_strlen_loop_0:
    mov dl, byte [r8+rcx] ; load current character
    cmp dl, 0 ; stop at terminator
    je __lambda_1_write_strlen_done_0
    inc rcx ; advance char counter
    jmp __lambda_1_write_strlen_loop_0
__lambda_1_write_strlen_done_0:
...
__lambda_3:
    ...
    mov r8, rdi ; keep string pointer
    xor rcx, rcx ; reset length counter
__lambda_3_write_strlen_loop_0:
    mov dl, byte [r8+rcx] ; load current character
    cmp dl, 0 ; stop at terminator
    je __lambda_3_write_strlen_done_0
    inc rcx ; advance char counter
    jmp __lambda_3_write_strlen_loop_0
__lambda_3_write_strlen_done_0:
    mov rsi, r8 ; buffer start
    mov rdx, rcx ; length to write
    mov rdi, 1 ; stdout fd
    call write ; invoke libc write
...
_start:
    ...
    mov rax, [rbp-16] ; load scalar from frame
    mov [r14+0], rax ; store variadic argument '__temp_9'
    mov rax, [rbp-32] ; load scalar from frame
    mov [r14+8], rax ; store variadic argument '__temp_10'
    mov rax, [rbp-48] ; load scalar from frame
    mov [r14+16], rax ; store variadic argument '__temp_11'
