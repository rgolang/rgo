...
_5_foo_deep_release:
    ...
    cmp rax, rbx
    jg _5_foo_release_skip_0
    mov rax, [r12-16] ; load _5_foo_release_field_0 env field
    mov [rbp-24], rax ; store value
    mov rdi, [rbp-24] ; load operand
    call release_heap_ptr ; release heap pointer
...
foo:
    ...
    lea rax, [_5_foo_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_5_foo_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_5_foo_deepcopy] ; load deep copy helper entry point
...
_9_lambda_unwrapper:
    ...
    leave ; unwind before named jump
    jmp _9_lambda
global _9_lambda_deep_release
_9_lambda_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
...
_start:
    ...
    mov qword [r12+32], rax ; heap size metadata
    lea rax, [_9_lambda_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_9_lambda_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_9_lambda_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 0 ; store num_remaining
    mov rax, r12 ; copy _9_lambda closure env_end to rax
