...
_5_foo_unwrapper:
    ...
    leave ; unwind before named jump
    jmp _5_foo ; jump to fully applied function
global _5_foo_release
_5_foo_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jl _5_foo_release_if_num_curried_lt_1
    mov rbx, [rbp-16] ; load env_end pointer for release
    mov rcx, [rbx-16] ; load field pointer
    mov rdi, rcx ; field release pointer
    call release_heap_ptr ; release heap pointer
    jmp _5_foo_release_done
_5_foo_release_if_num_curried_lt_1:
_5_foo_release_done:
    leave
    ret

global _5_foo_deepcopy
_5_foo_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 48 ; reserve stack space for locals
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jl _5_foo_deepcopy_if_num_curried_lt_1
    mov rbx, [rbp-16] ; load env_end pointer for copy
    mov rcx, [rbx-16] ; load field pointer
    test rcx, rcx ; skip null field
    je _5_foo_deepcopy_copy_field_null_0
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [rbx-16], rax ; store duplicated pointer
    jmp _5_foo_deepcopy_copy_field_done_1
_5_foo_deepcopy_copy_field_null_0:
    xor rax, rax ; null copy result
_5_foo_deepcopy_copy_field_done_1:
    mov [rbp-48], rax ; save evaluated scalar in frame
    jmp _5_foo_deepcopy_done
_5_foo_deepcopy_if_num_curried_lt_1:
_5_foo_deepcopy_done:
    leave
    ret
...
foo:
    ...
    mov rax, _5_foo_unwrapper ; load unwrapper entry point
    mov qword [rdx+0], rax ; store unwrapper entry in metadata
    mov rax, _5_foo_release ; load release helper entry point
    mov qword [rdx+8], rax ; store release pointer in metadata
    mov rax, _5_foo_deepcopy ; load deep copy helper entry point
    ...
    mov rdx, [rbp-96] ; load closure env_end for exec
    mov rax, [rdx+0] ; load closure unwrapper entry point
    sub rsp, 24 ; allocate temporary stack for closure state
    mov [rsp], rax ; save closure code pointer temporarily
    mov [rsp+8], rdx ; save closure env_end pointer temporarily
...
_start:
    ...
    mov qword [rdx+0], rax ; store unwrapper entry in metadata
    mov rax, _9_lambda_release ; load release helper entry point
    mov qword [rdx+8], rax ; store release pointer in metadata
    mov rax, _9_lambda_deepcopy ; load deep copy helper entry point
    mov qword [rdx+16], rax ; store deep copy pointer in metadata
    xor rax, rax ; zero num_curried metadata
    mov qword [rdx+40], rax ; store num_curried
