...
_2_foo_release:
    ...
    cmp rax, rbx
    jl _2_foo_release_if_num_curried_lt_1
...
_2_foo_deepcopy:
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jl _2_foo_deepcopy_if_num_curried_lt_1
    mov rbx, [rbp-16] ; load env_end pointer for copy
    mov rcx, [rbx-8] ; load field pointer
    test rcx, rcx ; skip null field
    je _2_foo_deepcopy_copy_field_null_0
    ...
    jmp _2_foo_deepcopy_copy_field_done_1
_2_foo_deepcopy_copy_field_null_0:
    xor rax, rax ; null copy result
_2_foo_deepcopy_copy_field_done_1:
    mov [rbp-48], rax ; save evaluated scalar in frame
    jmp _2_foo_deepcopy_done
...
_9_foo_unwrapper:
    ...
    call release_heap_ptr ; release closure environment
    leave ; unwind before named jump
    jmp _9_foo ; jump to fully applied function
global _9_foo_release
_9_foo_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
...
_6_foo:
    ...
    mov rax, _9_foo_unwrapper ; load unwrapper entry point
    mov qword [rdx+0], rax ; store unwrapper entry in metadata
    mov rax, _9_foo_release ; load release helper entry point
    mov qword [rdx+8], rax ; store release pointer in metadata
    mov rax, _9_foo_deepcopy ; load deep copy helper entry point
...
_6_foo_unwrapper:
    ...
    pop rdi ; restore scalar arg into register
    leave ; unwind before named jump
    jmp _6_foo ; jump to fully applied function
global _6_foo_release
_6_foo_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
...
foo:
    ...
    mov rax, _6_foo_unwrapper ; load unwrapper entry point
    mov qword [rdx+0], rax ; store unwrapper entry in metadata
    mov rax, _6_foo_release ; load release helper entry point
    mov qword [rdx+8], rax ; store release pointer in metadata
    mov rax, _6_foo_deepcopy ; load deep copy helper entry point
    mov qword [rdx+16], rax ; store deep copy pointer in metadata
    xor rax, rax ; zero num_curried metadata
    mov qword [rdx+40], rax ; store num_curried
    ...
    mov qword [rdx+0], rax ; store unwrapper entry in metadata
    mov rax, _2_foo_release ; load release helper entry point
    mov qword [rdx+8], rax ; store release pointer in metadata
    mov rax, _2_foo_deepcopy ; load deep copy helper entry point
    mov qword [rdx+16], rax ; store deep copy pointer in metadata
    xor rax, rax ; zero num_curried metadata
    mov qword [rdx+40], rax ; store num_curried
    sub rsp, 24 ; allocate temporary stack for closure state
