...
_8_lambda_unwrapper:
    ...
    leave ; unwind before named jump
    jmp _8_lambda
global _8_lambda_deep_release
_8_lambda_deep_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 16 ; reserve stack space for locals
    mov [rbp-8], rdi ; store env_end arg in frame
...
_6_lambda:
    ...
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    mov [rbp-8], rdi ; store foo arg in frame
    mov [rbp-16], rsi ; store p2 arg in frame
    mov rax, 9 ; mmap syscall
    ...
    lea rax, [_8_lambda_unwrapper] ; load unwrapper entry point
    mov qword [r12+0], rax ; store unwrapper entry in metadata
    lea rax, [_8_lambda_deep_release] ; load release helper entry point
    mov qword [r12+8], rax ; store release pointer in metadata
    lea rax, [_8_lambda_deepcopy] ; load deep copy helper entry point
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 0 ; store num_remaining
    mov rax, r12 ; copy _8_lambda closure env_end to rax
    mov [rbp-24], rax ; store value
    mov rbx, [rbp-8] ; load foo closure env_end pointer
    mov rax, [rbp-16] ; load operand
...
_6_lambda_unwrapper:
    ...
    mov [rbp-8], rdi ; store env_end arg in frame
    mov r12, [rbp-8] ; load operand
    mov rax, [r12-16] ; load foo env field
    mov [rbp-16], rax ; store value
    mov rax, [r12-8] ; load p2 env field
    mov [rbp-24], rax ; store value
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    mov rax, [rbp-24] ; load operand
...
_6_lambda_deep_release:
    ...
    mov rax, [r12-16] ; load _6_lambda_release_field_0 env field
    mov [rbp-24], rax ; store value
    mov rdi, [rbp-24] ; load operand
    call release_heap_ptr ; release heap pointer
_6_lambda_release_skip_0:
    ...
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _6_lambda_release_skip_1
    mov rax, [r12-8] ; load _6_lambda_release_field_1 env field
    mov [rbp-32], rax ; store value
    mov rdi, [rbp-32] ; load operand
    call release_heap_ptr ; release heap pointer
_6_lambda_release_skip_1:
    mov rdi, r12 ; use pinned __env_end env_end pointer
    call release_heap_ptr ; release __env_end closure environment
    leave
    ret
...
_6_lambda_deepcopy_skip_0:
    ...
    mov rbx, 0 ; operand literal
    cmp rax, rbx
    jg _6_lambda_deepcopy_skip_1
    mov rcx, [r12-8] ; load field pointer
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [r12-8], rax ; store duplicated pointer
    mov [rbp-32], rax ; store value
_6_lambda_deepcopy_skip_1:
    leave
    ret
...
_start:
    ...
    mov qword [r12+16], rax ; store deep copy pointer in metadata
    mov qword [r12+40], 0 ; store num_remaining
    mov rax, r12 ; copy _11_ closure env_end to rax
    mov [rbp-24], rax ; store value
    mov rax, 9 ; mmap syscall
    xor rdi, rdi ; addr hint for kernel base selection
    mov rsi, 56 ; length for allocation
