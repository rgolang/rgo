...
print1_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp print1 ; jump into actual function
global print2
print2:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
...
print2_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp print2 ; jump into actual function
global p1
p1:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
...
p1_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp p1 ; jump into actual function
global p2
p2:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
...
p2_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp p2 ; jump into actual function
global __1_1
__1_1:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
...
__1_1_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp __1_1 ; jump into actual function
global _1
_1:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    mov rax, __1_1_closure_entry ; load wrapper entry point
    sub rsp, 24 ; allocate temporary stack for closure state
    mov [rsp], rax ; save closure code pointer temporarily
    mov [rsp+8], rdx ; save closure env_end pointer temporarily
    mov rax, [rsp] ; restore closure code pointer
    ...
    mov rax, p2_closure_entry ; load wrapper entry point
...
_start:
    ...
    mov qword [rdx+16], 1 ; pointer count metadata
    mov qword [rdx+24], 8 ; closure env pointer slot offset
    mov rax, p1_closure_entry ; load wrapper entry point
    mov [rbp-32], rax ; update closure code pointer
    mov [rbp-24], rdx ; update closure environment pointer
    mov rax, [rbp-16] ; load closure code pointer
    mov rdx, [rbp-8] ; load closure env_end pointer
