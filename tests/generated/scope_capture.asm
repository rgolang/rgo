...
global _3_print_sum_unwrapper
_3_print_sum_unwrapper:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    jmp _3_print_sum ; jump to fully applied function
global _3_print_sum_release
_3_print_sum_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    cmp rax, rbx
    jl _3_print_sum_release_if_num_curried_lt_1
    ...
    jmp _3_print_sum_release_done
_3_print_sum_release_if_num_curried_lt_1:
_3_print_sum_release_done:
    leave
    ret

global _3_print_sum_deepcopy
_3_print_sum_deepcopy:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 48 ; reserve stack space for locals
    mov [rbp-16], rdi ; store scalar arg in frame
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jl _3_print_sum_deepcopy_if_num_curried_lt_1
    mov rbx, [rbp-16] ; load env_end pointer for copy
    mov rcx, [rbx-16] ; load field pointer
    test rcx, rcx ; skip null field
    je _3_print_sum_deepcopy_copy_field_null_0
    ...
    jmp _3_print_sum_deepcopy_copy_field_done_1
_3_print_sum_deepcopy_copy_field_null_0:
    xor rax, rax ; null copy result
_3_print_sum_deepcopy_copy_field_done_1:
    mov [rbp-48], rax ; save evaluated scalar in frame
    jmp _3_print_sum_deepcopy_done
...
print_sum:
    ...
    mov rax, _3_print_sum_unwrapper ; load unwrapper entry point
    mov qword [rdx+0], rax ; store unwrapper entry in metadata
    mov rax, _3_print_sum_release ; load release helper entry point
    mov qword [rdx+8], rax ; store release pointer in metadata
    mov rax, _3_print_sum_deepcopy ; load deep copy helper entry point
...
print_sum_unwrapper:
    ...
    pop rsi ; restore closure env_end into register
    leave ; unwind before named jump
    jmp print_sum ; jump to fully applied function
global print_sum_release
print_sum_release:
    push rbp ; save executor frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 32 ; reserve stack space for locals
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jl print_sum_release_if_num_curried_lt_1
    mov rbx, [rbp-16] ; load env_end pointer for release
    mov rcx, [rbx-8] ; load field pointer
    mov rdi, rcx ; field release pointer
    call release_heap_ptr ; release heap pointer
    jmp print_sum_release_done
...
print_sum_deepcopy:
    ...
    mov rbx, 1 ; operand literal
    cmp rax, rbx
    jl print_sum_deepcopy_if_num_curried_lt_1
    mov rbx, [rbp-16] ; load env_end pointer for copy
    mov rcx, [rbx-8] ; load field pointer
    test rcx, rcx ; skip null field
    je print_sum_deepcopy_copy_field_null_0
    mov rdi, rcx ; copy pointer argument for deepcopy
    call deepcopy_heap_ptr ; duplicate heap pointer
    mov [rbx-8], rax ; store duplicated pointer
    jmp print_sum_deepcopy_copy_field_done_1
print_sum_deepcopy_copy_field_null_0:
    xor rax, rax ; null copy result
print_sum_deepcopy_copy_field_done_1:
    mov [rbp-48], rax ; save evaluated scalar in frame
    jmp print_sum_deepcopy_done
