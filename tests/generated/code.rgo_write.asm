...
_start:
    ...
    jmp rgo_write ; jump to fully applied function
global rgo_write
rgo_write:
...
write_strlen_loop:
    ...
    je write_strlen_done
    inc rcx ; advance char counter
    jmp write_strlen_loop
write_strlen_done:
    ...
    pop rsi ; restore continuation code pointer
    mov rax, rsi ; continuation entry point
    mov rdi, rdx ; pass env_end pointer to continuation
    leave ; epilogue: unwind before jump
