...
zero_closure_entry:
    ...
    mov rdx, [rbx+16] ; load scalar param from env
    push rdx ; preserve parameter register
    ...
    jmp zero ; jump into actual function
global one
one:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
...
one_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp one ; jump into actual function
global two
two:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    mov rax, _two_1_closure_entry ; load wrapper entry point
    sub rsp, 24 ; allocate temporary stack for closure state
    mov [rsp], rax ; save closure code pointer temporarily
    mov [rsp+8], rdx ; save closure env_end pointer temporarily
...
two_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp two ; jump into actual function
global _two_1
_two_1:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
...
_two_1_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp _two_1 ; jump into actual function
global three
three:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    mov qword [rdx+24], 8 ; closure env pointer slot offset
    mov qword [rdx+32], 24 ; closure env pointer slot offset
    mov rax, _three_1_closure_entry ; load wrapper entry point
    sub rsp, 24 ; allocate temporary stack for closure state
    mov [rsp], rax ; save closure code pointer temporarily
    mov [rsp+8], rdx ; save closure env_end pointer temporarily
    mov rbx, [rsp+8] ; original closure env_end pointer
...
three_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp three ; jump into actual function
global _three__three_1_1
_three__three_1_1:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
    sub rsp, 48 ; reserve stack space for locals
    ...
    leave ; unwind before calling closure
    jmp rax ; jump into fully applied closure
_three__three_1_1_closure_entry:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish wrapper frame
    sub rsp, 16 ; reserve space for env metadata scratch
    mov [rbp-8], rdi ; stash env_end pointer for release
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp _three__three_1_1 ; jump into actual function
global _three_1
_three_1:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    mov qword [rdx+24], 8 ; closure env pointer slot offset
    mov qword [rdx+32], 24 ; closure env pointer slot offset
    mov rax, _three__three_1_1_closure_entry ; load wrapper entry point
    sub rsp, 24 ; allocate temporary stack for closure state
    mov [rsp], rax ; save closure code pointer temporarily
    mov [rsp+8], rdx ; save closure env_end pointer temporarily
    mov rbx, [rsp+8] ; original closure env_end pointer
...
_three_1_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp _three_1 ; jump into actual function
global _1
_1:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
...
_1_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp _1 ; jump into actual function
global __4_2
__4_2:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
...
__4_2_closure_entry:
    ...
    pop rbx ; restore saved base register
    leave ; epilogue: restore rbp of caller
    jmp __4_2 ; jump into actual function
global _4
_4:
    push rbp ; save caller frame pointer
    mov rbp, rsp ; establish new frame base
    ...
    mov rax, __4_2_closure_entry ; load wrapper entry point
    sub rsp, 24 ; allocate temporary stack for closure state
    mov [rsp], rax ; save closure code pointer temporarily
    mov [rsp+8], rdx ; save closure env_end pointer temporarily
    mov rax, [rsp] ; restore closure code pointer
...
_start:
    ...
    mov rax, _1_closure_entry ; load wrapper entry point
    sub rsp, 24 ; allocate temporary stack for closure state
    mov [rsp], rax ; save closure code pointer temporarily
    mov [rsp+8], rdx ; save closure env_end pointer temporarily
    mov rax, [rsp] ; restore closure code pointer
